...
.CHECK_EQUAL (threetree.size(), 3)
  ...
  .CHECK_EQUAL (test1.size(), 2)
  .CHECK_EQUAL (test2.size(), 1)
  .CHECK_EQUAL (test3.size(), 0)

  .CHECK_OK    (test1.insert(13))
  .CHECK_OK    (test1.insert(9))
  .CHECK_EQUAL (test1.size() , 4)
  .CHECK_EQUAL (test1.contains(13), true)
  .CHECK_EQUAL (test1.contains(17), false)
  .CHECK_EQUAL (test1.size() ,4)

  .CHECK_OK    (test2.insert(23))
  .CHECK_EQUAL (test2.root(), 9)
